...
export const Add_teacher = (props) => {
    ...
    const searchCountry = (event) => {
        setTimeout(() => {
            ...
            //     _filtered_papers = [...papers];
            // }
            // else {
            //     _filtered_papers = papers.filter((paper) => {
            //         return paper.paper_code.toLowerCase().startsWith(event.query.toLowerCase());
            //     });
            // }
            let _filtered_papers = [];
        ...
        return (
            <div>
                <Button label="No" icon="pi pi-times" onClick={() =>{
                    props.close_overlay() ; onHide(name)
